...
# 🔥 [NameError 해결 완벽 조치] 컬러링 함수를 테이블 출력부보다 훨씬 위쪽에 선언하여 인식 오류를 원천 차단했습니다.
...
with tab5:
    ...
    for row in sector_definition_data:
        t = row["티커"]
        target_d = sec_res_v4.get(t, {"currently_close":0, "현재가":0, "일간변동(%)":0})
...
# --- 탭 7: 통화정책 & 글로벌 핵심 캘린더 ---
with tab7:
    str_visual.header("🗓️ 글로벌 중앙은행 핵심 경제지표 캘린더 스케줄러 (실시간 동기화)")
