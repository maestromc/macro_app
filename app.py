...
with str_visual.spinner("선택하신 기준일의 매크로 및 주가 데이터를 보정하는 중..."):
    ...
    for asset_name, ticker in [("달러 인덱스", "DX-Y.NYB"), ("금 선물", "GC=F"), ("SK하이닉스", "000660.KS")]:
        if macro_results.get(asset_name, {}).get("현재가", 0) == 0 and asset_name in macro_results:
            ...
            if not bk_df.empty:
                macro_results[asset_name]["currently"] = round(float(bk_df['Close'].iloc[-1]), 2)
...
# --- 탭 7: 글로벌 통화정책 캘린더 (창 크기 5배 확장 완료!) ---
with tab7:
    ...
    # 🚀 창 크기 확장을 위해 height 값을 3000에서 15000으로 5배 상향 조정했습니다!
    tradingview_widget_js = """
    <div class="tradingview-widget-container" style="height:15000px; width:100%;">
    ...
    """
    components.html(tradingview_widget_js, height=15000)
...
with tab10:
    ...
    with ln2:
        ...
        str_visual.markdown("- [🧠 오픈AI 클로드 (Claude)](https://claude.ai)") # 치명적 오타 mrkdown -> markdown으로 완벽 복구!
