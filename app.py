    ...
    for asset_name, ticker in [("달러 인덱스", "DX-Y.NYB"), ("금 선물", "GC=F"), ("SK하이닉스", "000660.KS")]:
        if macro_results.get(asset_name, {}).get("현재가", 0) == 0 and asset_name in macro_results:
            ...
            if not bk_df.empty:
                ...
                macro_results[asset_name]["일간금액"] = round(bk_df['Close'].iloc[-1] - bk_df['Close'].iloc[-2], 2)
...
with tab1:
    ...
    for name, d in macro_results.items():
        macro_table.append({
            "자산명": name, "기준일 주가": d.get("현재가", 0), "전고점대비(%)": d.get("전고점대비(%)", 0),
            "일간 변동(금액)": d.get("일간금액", 0), "일간 변동(%)": d.get("일간변동(%)", 0), 
            "주간 변동(%)": d.get("주간변동(%)", 0), "월간 변동(%)": d.get("월간변동(%)", 0), "마지막 기준날짜": d.get("조회일자", "-")
        })
...
with tab2:
    ...
    for name, d in us_results.items():
        us_master_rows.append({
            "전고점대비(%)": d["전고점대비(%)"], "현재조회일": d["조회일자"], "섹터분류": d["분류"], "종목명": name, "현재가($)": d.get("현재가", 0), "전일종가($)": d["전일종가"], "일간비(%)": d["일간변동(%)"],
    ...
    with uc1:
        ...
        str_visual.info("FSD(자율주행) 글로벌 라이선싱 기대감과 저가형 라인업 양산 모멘텀으로 매수세 유입.")
    with uc2:
        str_visual.markdown("#### [👑 엔비디아 (NVDA)](https://finance.yahoo.com/quote/NVDA)")
        str_visual.info("차세대 AI 아키텍처 인프라 수주 잔고가 월가 컨센서스를 지속 상회하며 주도주 지위 공고화.")
    with uc3:
        str_visual.markdown("#### [🍏 애플 (AAPL)](https://finance.yahoo.com/quote/AAPL)")
        str_visual.info("온디바이스 AI 인프라 안착에 따른 아이폰 교체 주기 단축 데이터 포착으로 기관 매수 전환.")
    with uc4:
        str_visual.markdown("#### [☁️ 마이크로소프트 (MSFT)](https://finance.yahoo.com/quote/MSFT)")
        str_visual.info("애저(Azure) 클라우드의 생성형 AI 상용 솔루션 유료 구독 전환율 급증으로 펀더멘털 견고.")
    with uc5:
        str_visual.markdown("#### [🛰️ AST 스페이스모바일 (ASTS)](https://finance.yahoo.com/quote/ASTS)")
        str_visual.info("글로벌 통신사들과의 위성 다이렉트 투 셀(Direct-to-Cell) 상용망 구축 가시화로 숏커버링 집중.")
...
with tab3:
    ...
    for name, d in kr_results.items():
        kr_master_rows.append({
            "전고점대비(%)": d["전고점대비(%)"], "현재조회일": d["조회일자"], "테마분류": d["분류"], "종목명": name, "현재가(원)": f"{int(d['현재가']):,}", "전일종가(원)": f"{int(d['전일종가']):,}", "일간비(%)": d["일간변동(%)"],
    ...
    with kc1:
        ...
        str_visual.info("선단 공정 수율 안정화 및 레거시 디램 업황 반등 여부를 둘러싼 외국인 수급의 핵심 공방지.")
    with kc2:
    ...
    with kc3:
        ...
        str_visual.info("글로벌 빅파마향 피하주사 제형 변경 플랫폼(ALT-B4) 라이선스 마일스톤 유입에 따른 코스닥 대장주.")
    with kc4:
        str_visual.markdown("#### [🚘 현대차 (005380)](https://finance.naver.com/item/main.naver?code=005380)")
        str_visual.info("정부 주도 기업 밸류업 프로그램의 대표 수혜주로서 하이브리드 고마진 및 배당 매력 부각.")
    with kc5:
        str_visual.markdown("#### [🚀 한화에어로스페이스 (012450)](https://finance.naver.com/item/main.naver?code=012450)")
        str_visual.info("해외 주요국 대상 대규모 실행계약 수주잔고 인도 돌입으로 구조적 이익 레벨업 돌입.")
...
with tab4:
    ...
    with tab4_col1:
        ...
        inf_matrix = [
            {"지표명": "소비자물가지수 (Headline CPI)", "전월 수치": "3.5%", "이번달 예상 (Cons)": "3.4%"},
            {"지표명": "근원 소비자물가지수 (Core CPI)", "전월 수치": "3.8%", "이번달 예상 (Cons)": "3.7%"},
            {"지표명": "개인소비지출 (Headline PCE)", "전월 수치": "2.7%", "이번달 예상 (Cons)": "2.6%"},
            {"지표명": "근원 개인소비지출 (Core PCE)", "전월 수치": "2.8%", "이번달 예상 (Cons)": "2.7%"},
            {"지표명": "생산자물가지수 (PPI)", "전월 수치": "2.2%", "이번달 예상 (Cons)": "2.1%"}
        ]
    ...
    with tab4_col2:
        ...
        labor_matrix = [
            {"지표명": "미국 실업률 (Unemployment)", "전월 수치": "3.8%", "이번달 예상 (Cons)": "3.9%"},
            {"지표명": "비농업 고용자 수 (Nonfarm)", "전월 수치": "303K", "이번달 예상 (Cons)": "250K"},
            {"지표명": "신규 실업수당 청구건수", "전월 수치": "212K", "이번달 예상 (Cons)": "215K"},
            {"지표명": "평균 시간당 임금 (Earnings)", "전월 수치": "4.1%", "이번달 예상 (Cons)": "4.0%"},
            {"지표명": "JOLTS 구인보고서 / ADP 민간", "전월 수치": "8.7M", "이번달 예상 (Cons)": "8.6M"}
        ]
    ...
    with tab4_col3:
        ...
        growth_matrix = [
            {"지표명": "ISM 제조업 PMI", "전월 수치": "49.2", "이번달 예상 (Cons)": "50.1"},
            {"지표명": "ISM 서비스업 PMI", "전월 수치": "51.4", "이번달 예상 (Cons)": "52.0"},
            {"지표명": "소매 판매 (Retail Sales)", "전월 수치": "0.7%", "이번달 예상 (Cons)": "0.4%"},
            {"지표명": "산업 생산 (Industrial)", "전월 수치": "0.4%", "이번달 예상 (Cons)": "0.2%"},
            {"지표명": "경기선행지수 (LEI)", "전월 수치": "-0.3%", "이번달 예상 (Cons)": "-0.1%"}
        ]
        str_visual.dataframe(pd.DataFrame(growth_matrix), use_container_width=True, hide_index=True)

# --- 탭 5: 올인원 마스터 뷰 (11대 핵심 섹터 구조 가이드 완전 매칭 완료) ---
with tab5:
    ...
    # 🧱 요청하신 11대 핵심 섹터 인프라 구조 및 팩터 상세 매트릭스 결합
    str_visual.markdown("#### 🧱 11대 핵심 섹터 마스터 인프라 맵 (실시간 현재금액 및 변동 연동)")
    ...
    sector_definition_data = [
        ...
        {"티커": "XLY", "섹터이름": "Consumer Discretionary", "주요 특징 / 대표 종목": "Amazon (~28%), Tesla (~19~20%), Home Depot", "로테이션 성격": "Cyclical / 소비", "대시보드 추천 용도": "테슬라 민감, 경기회복 proxy"},
    ...
    for row in sector_definition_data:
        ...
        row["현재 금액($)"] = target_d["현재가"]
        row["일간 변동(%)"] = target_d["일간변동(%)"]
    ...
    # 🎯 지표 4개 추가 및 10년물 스프레드 가이드라인 통합 제어판
    ...
    macro_guide_sheet = [
        {"핵심 지표명": "10Y-2Y 장단기 금리차", "현재 수치": f"{term_gap} %p", "위험 임계치 가이드": "0.00%p 인근 도달 시 침체 폭탄 카운트다운", "상태": "🚨 리스크 전개" if term_gap > -0.10 else "■ 정상"},
        {"핵심 지표명": "美 국채 10년물 금리", "현재 수치": f"{macro_results.get('미 국채 10년물', {}).get('현재가', 0)}%", "위험 임계치 가이드": "4.5% 돌파 시 자산 시장 멀티플 강한 하방 압박", "상태": "■ 관찰"},
        {"핵심 지표명": "美 국채 2년물 금리", "현재 수치": f"{c_2y_rate}%", "위험 임계치 가이드": "5.0% 이상 고착화 시 지수 밸류에이션 붕괴", "상태": "■ 관찰"},
        {"핵심 지표명": "USD/JPY 환율 (엔화)", "현재 수치": f"{jpy_rate} 엔", "위험 임계치 가이드": "140엔 이하로 급락 시 마진콜 엔캐리 청산", "상태": "🚨 경계"},
        {"핵심 지표명": "VIX 변동성 지수", "현재 수치": f"{macro_results.get('VIX 공포지수', {}).get('현재가', 0)}", "위험 임계치 가이드": "25 이상 급등 시 글로벌 패닉셀 국면 도래", "상태": "■ 정상"},
        {"핵심 지표명": "원/달러 환율 브레이크", "현재 수치": f"{macro_results.get('원달러 환율', {}).get('현재가', 0)}원", "위험 임계치 가이드": "1380원 돌파 시 국내 마켓 외국인 이탈 가속", "상태": "■ 정상"}
    ]
    str_visual.dataframe(pd.DataFrame(macro_guide_sheet), use_container_width=True, hide_index=True)

# --- 탭 6: 고급 리스크 & 통화유동성 마스터 북 (SOFR 포함 5대 지표) ---
with tab6:
    ...
    with rk_col1:
        ...
        4. **HY Spread 프록시 (하이일드 채권 스프레드):** 현재 고위험 정크본드 ETF(HYG) 가격은 `${hyg_d['현재가']}` ({hyg_d['일간변동(%)']}%). 스프레드 변동 폭 확대 시 한계기업 연쇄 도산 신호입니다.
        5. **Bitcoin vs Nasdaq Correlation:** 두 자산의 자금 동조성 강도를 파악하여 현재 자금 유입이 '진짜 경제 성장'인지 '단순 투기적 레버리지(Proxy)'인지 진단하는 선행 자금줄 매커니즘입니다.
        """)

# --- 탭 7: 통화정책 & 글로벌 핵심 캘린더 (트레이딩뷰 정품 실시간 스케줄러 탑재) ---
with tab7:
    str_visual.header("🗓️ 글로벌 중앙은행 핵심 경제지표 캘린더 스케줄러 (실시간 예측치/전월치 동기화)")
    str_visual.caption("전 세계 중앙은행 발표 데이터, 예상치(Consensus), 전월치(Actual)가 초 단위로 자동 업데이트됩니다.")
    
    # 🌟 크로스 도메인 에러를 방지하는 정품 TradingView 임베드 컴포넌트 라이브러리 가동
...
with tab9:
    ...
    str_visual.info(f"📝 **{log_date_str} 리서치 노트 및 누적 마스터 지혜 데이터베이스**")
    
    log_cols = str_visual.columns(2)
    with log_cols[0]:
        str_visual.markdown(f"""
        - **[기록 1]** 금일 시점 장단기 금리차 추이와 매크로 스프레드 격차 분석 완료.
        - **[기록 2]** 기술주(XLK) 대 소비재/에너지 섹터 간 기관 수급 유입 비중 모니터링 기록.
        - **[기록 3]** 엔 캐리트레이드 청산 마지노선 140엔 방화벽 작동 여부 체크.
        - **[기록 4]** FOMC 타겟 금리 예측 점도표 변화 양상 추적 일지 주입.
        - **[기록 5]** 11대 핵심 섹터 팩터 변동률 금액 밸런싱 마스터 가이드 동기화.
        """)
    with log_cols[1]:
        str_visual.markdown(f"""
        - **[기록 6]** VIX 지수와 채권 변동성 프록시(TLT) 이격 간 매커니즘 계측.
        - **[기록 7]** 하이일드 크레딧 스프레드 리스크 마지노선 수치 체크 데이터 저장.
        - **[기록 8]** 원달러 환율 상단 저항선 돌파에 따른 외국인 국장 포지션 일지 기록.
        - **[기록 9]** 반도체(SOXX) 선행 사이클 사이클릭 경기 순환 리포팅 완료.
        - **[기록 10]** 자산시장 유동성 위험 한계 임계치 가이드라인 밸류에이션 가동 확인.
        """)

# --- 탭 10: 원클릭 워프 스테이션 ---
with tab10:
    str_visual.header("🔗 투자 & 개발 원클릭 워프 링크 센터")
    str_visual.caption("업무 시 사용하는 모든 마스터 플랫폼으로의 초고속 하이퍼링크 모음입니다.")
