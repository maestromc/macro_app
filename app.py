...
with tab4:
    ...
    with mid_left:
        ...
        sector_mockup = [
            ...
            {"섹터구분": "XLU (유틸리티)", "▼ 약세 반전", "방어적 자금 일시 이탈"},
            {"섹터구분": "XLE (에너지)", "■ 기간 조정", "유가 80달러선 지지선 탐색"}
        ]
    ...
    with mid_right:
        ...
        macro_all_in_one = [
            ...
            {"생산자물가지수 (PPI)", "도매물가 컨센서스 부합 추세", "소비자물가 선행 지표로 안정적"},
            {"Global PMI (제조업)", "50.1 경기 확장선 상회 여부 주시", "글로벌 경기 하방 압력 방어 신호"},
            {"10Y-2Y 장단기 금리차", "스프레드 역전 폭 축소 과정 체크", "정상화 도래 시 경기둔화 헷지 준비"}
        ]
